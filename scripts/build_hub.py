...
def stale_thresholds() -> dict[str, int]:
    try: registry = load(REGISTRY)
    except (OSError, json.JSONDecodeError): return {}
    return {str(x.get("id")): int(x.get("stale_after_hours", DEFAULT_STALE_HOURS)) for x in registry.get("repositories", []) if x.get("id")}


def repo_health(doc: dict, now_dt: datetime, stale_hours: int) -> dict:
...
def aggregate(docs: list[dict]) -> dict:
    ...
    for doc in sorted(docs,key=lambda d:d["repo"]["full_name"].lower()):
        ...
        for name in COLLECTIONS:
            for item in doc.get(name,[]): result[name].append({"repo_id":repo["id"],"repo_full_name":repo["full_name"],**item})
